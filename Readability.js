...
function Readability(doc, options) {
  ...
  this._maxElemsToParse = options.maxElemsToParse ||
    this.DEFAULT_MAX_ELEMS_TO_PARSE;
  this._nbTopCandidates = options.nbTopCandidates ||
    this.DEFAULT_N_TOP_CANDIDATES;
...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    // Commas as used in Latin, Sindhi, Chinese and constious other scripts.
    // see: https://en.wikipedia.org/wiki/Comma#Comma_constiants
    commas: /\u002C|\u060C|\uFE50|\uFE10|\uFE11|\u2E41|\u2E34|\u2E32|\uFF0C/g,
  ...
  _fixRelativeUris: function (articleContent) {
    ...
    let medias = this._getAllNodesWithTag(articleContent, [
      "img",
      "picture",
      "figure",
      "video",
  ...
  _getArticleTitle: function () {
    ...
    } catch (_e) { /* ignore exceptions setting the title. */ }
  ...
  _prepArticle: function (articleContent) {
    ...
    this._forEachNode(
      ...
      function (table) {
        let tbody = this._hasSingleTagInsideElement(table, "TBODY")
  ...
  _textSimilarity: function (textA, textB) {
    let tokensA = textA.toLowerCase().split(this.REGEXPS.tokenize).filter(
      Boolean,
    );
    let tokensB = textB.toLowerCase().split(this.REGEXPS.tokenize).filter(
      Boolean,
    );
  ...
  /***
   * grabArticle - Using a constiety of metrics (content score, classname, element types), find the content that is
  ...
  _grabArticle: function (page) {
    ...
    while (true) {
      ...
      let stripUnlikelyCandidates = this._flagIsActive(
      ...
      while (node) {
        ...
        if (stripUnlikelyCandidates) {
          ...
          if (this.UNLIKELY_ROLES.includes(node.getAttribute("role"))) {
            this.log(
              "Removing content with role " + node.getAttribute("role") +
                " - " + matchString,
            );
            node = this._removeAndGetNext(node);
            continue;
      ...
      for (let c = 0, cl = candidates.length; c < cl; c += 1) {
        ...
        let candidateScore = candidate.readability.contentScore *
        ...
        for (let i = 1; i < topCandidates.length; i++) {
          if (
            topCandidates[i].readability.contentScore /
                topCandidate.readability.contentScore >= 0.75
          ) {
      ...
      if (parseSuccessful) {
        ...
        let ancestors = [parentOfTopCandidate, topCandidate].concat(
  ...
  _getJSONLD: function (doc) {
    let scripts = this._getAllNodesWithTag(doc, ["script"]);
    ...
    this._forEachNode(scripts, function (jsonLdElement) {
      if (
        ...
        try {
          ...
          let content = jsonLdElement.textContent.replace(
            /^\s*<!\[CDATA\[|\]\]>\s*$/g,
            "",
          );
          ...
          if (
            ...
            let nameMatches = this._textSimilarity(parsed.name, title) > 0.75;
            let headlineMatches =
  ...
  _unwrapNoscriptImages: function (doc) {
    ...
    this._forEachNode(noscripts, function (noscript) {
      ...
      let prevElement = noscript.previousElementSibling;
      if (prevElement && this._isSingleImage(prevElement)) {
        ...
        let newImg = tmp.getElementsByTagName("img")[0];
        for (let i = 0; i < prevImg.attributes.length; i++) {
          let attr = prevImg.attributes[i];
          if (attr.value === "") {
            continue;
          }
  ...
  _isPhrasingContent: function (node) {
    return node.nodeType === this.TEXT_NODE ||
      ...
      ((node.tagName === "A" || node.tagName === "DEL" ||
        node.tagName === "INS") &&
    ...
    this._forEachNode(
      ...
      function (elem) {
        ...
        for (let j = 0; j < elem.attributes.length; j++) {
          ...
          if (
            attr.name === "src" || attr.name === "srcset" || attr.name === "alt"
          ) {
            continue;
          }
          let copyTo = null;
  ...
  _cleanConditionally: function (e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
      ...
      if (!isList) {
        let listLength = 0;
        let listNodes = this._getAllNodesWithTag(node, ["ul", "ol"]);
        this._forEachNode(
          listNodes,
      ...
      if (this._getCharCount(node, ",") < 10) {
        ...
        let headingDensity = this._getTextDensity(node, [
          "h1",
          "h2",
          "h3",
          "h4",
        ...
        let embedCount = 0;
        let embeds = this._getAllNodesWithTag(node, [
          "object",
          "embed",
          "iframe",
        ]);
        ...
        let haveToRemove =
          ...
          (!isList && headingDensity < 0.9 && contentLength < 25 &&
            (img === 0 || img > 2) && !this._hasAncestorTag(node, "figure")) ||
